#include "DSLoaders/Loader.h"

//#include <debug.h>
#include <algorithm>
#include <math.h>

void Loader::findMinMax(int voxelCount, unsigned bytesPerValue, uint8_t *data, unsigned &u_min, unsigned &u_max)
{
    for(int i=0; i<voxelCount; i++) {
		...
		u_min = std::min(u_min, raw_val);
		u_max = std::max(u_max, raw_val);
    }
}
...
{
    int bytesPerValue = ceil(bitDepth/8.);
    
    unsigned int rangeMax = pow(2, bitDepth);
