...
uint8_t *DDSLoader::loadFile(const std::string &filename)
{
    unsigned int components;
    uint8_t *raw = readPVMvolume(filename.c_str(), &width, &height, &depth, &components);
...
bool DDSLoader::writeRawFile(const std::string &filename, uint8_t *pVoldata, uint32_t VoldataSize)
{
	FILE *file;

	if ((file = fopen(filename.c_str(), "wb")) == NULL)
	{
		printf("<DDSLoader::writeRawFile>Error: Unable to open output raw file to write: %s",filename.c_str());
		return(false);
	}

	if (VoldataSize != fwrite(pVoldata, sizeof(char), VoldataSize, file))
	{
		printf("<DDSLoader::writeRawFile>Error: Unable to write all data to file.");
	}
